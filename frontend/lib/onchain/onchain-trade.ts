...
export async function invokeStarknetCallsFromWallet(
  ...
  const requiresStrictWalletInvoke =
    ...
    normalizedCalls.some(
      (call) => call.entrypoint === "submit_private_action" || call.entrypoint === "submit_action"
    )
...
export async function readStarknetErc20AllowanceFromWallet(
  ...
  await ensureStarknetAccounts(injected, { prompt: false })
  const chainId = await ensureStarknetSepolia(injected)
...
export async function readStarknetShieldedPoolFixedAmountFromWallet(
...
export async function readStarknetShieldedPoolNoteDepositTimestampFromWallet(
  executorAddress: string,
...
export async function readStarknetShieldedPoolCurrentRootFromWallet(
  executorAddress: string,
  providerHint: StarknetWalletHint = "starknet"
): Promise<string | null> {
  const executor = executorAddress.trim()
...
export async function sendEvmTransactionFromWallet(tx: {
  ...
  const gasMode = (process.env.NEXT_PUBLIC_EVM_GAS_MODE || "auto").toLowerCase()
  const useFixedGas = gasMode === "fixed" || gasMode === "manual"
  if (
    useFixedGas &&
    typeof tx.gas_limit === "number" &&
    Number.isFinite(tx.gas_limit) &&
    tx.gas_limit > 0
  ) {
...
function getInjectedStarknet(providerHint: StarknetWalletHint): InjectedStarknet | null {
  ...
  const connectedProvider =
    (hasConnectedAddress(argent) && argent) ||
    (hasConnectedAddress(braavos) && braavos) ||
    (hasConnectedAddress(defaultProvider) && defaultProvider) ||
    null
  ...
  return connectedProvider || defaultProvider || argent || braavos
}
 ...
 * @remarks May trigger network calls, Hide Mode processing, or local state mutations.
 */
async function ensureStarknetAccounts(
  ...
  if (injected.request) {
    ...
    for (const payload of attempts) {
      try {
        const result = await requestStarknet(injected, payload)
        rememberInjectedStarknetAccount(injected, result)
        if (injected.selectedAddress || injected.account?.address) return
      } catch {
        // continue
      }
...
async function readStarknetChainId(injected: InjectedStarknet): Promise<string | undefined> {
  const fromCurrent = parseChainId(injected.chainId)
  if (fromCurrent) {
  ...
  const fromCurrentObject =
    parseChainId((injected as { chain_id?: unknown }).chain_id) ||
    parseChainId((injected as { network?: unknown }).network)
  ...
  const fromAccountField =
    parseChainId((injected.account as { chainId?: unknown } | undefined)?.chainId) ||
    parseChainId((injected.account as { chain_id?: unknown } | undefined)?.chain_id) ||
    parseChainId((injected.account as { network?: unknown } | undefined)?.network)
  if (fromAccountField) {
  ...
  const fromProviderField =
    parseChainId((injected.provider as { chainId?: unknown } | undefined)?.chainId) ||
    parseChainId((injected.provider as { chain_id?: unknown } | undefined)?.chain_id) ||
    parseChainId((injected.provider as { network?: unknown } | undefined)?.network)
...
function parseChainId(value: unknown): string | null {
  ...
  if (typeof value === "object" && value) {
    return (
      ...
      parseChainId((value as { chain_id?: unknown }).chain_id) ||
      parseChainId((value as { network?: unknown }).network) ||
      parseChainId((value as { networkId?: unknown }).networkId) ||
      parseChainId((value as { network_id?: unknown }).network_id) ||
      parseChainId((value as { chain?: unknown }).chain) ||
      parseChainId((value as { value?: unknown }).value) ||
      parseChainId((value as { result?: unknown }).result) ||
      parseChainId((value as { data?: unknown }).data) ||
      parseChainId((value as { payload?: unknown }).payload)
    )
  }
  return null
}

function rememberInjectedStarknetAccount(injected: InjectedStarknet, result: unknown): string | null {
  const address =
    parseInjectedStarknetAddress(result) ||
    parseInjectedStarknetAddress(injected.selectedAddress) ||
    parseInjectedStarknetAddress(injected.account?.address)
  if (address) {
...
function parseInjectedStarknetAddress(value: unknown): string | null {
  if (typeof value === "string") {
    const trimmed = value.trim()
  ...
  if (Array.isArray(value)) {
    for (const item of value) {
      const parsed = parseInjectedStarknetAddress(item)
  ...
  if (typeof value === "object" && value) {
    return (
      parseInjectedStarknetAddress((value as { address?: unknown }).address) ||
      parseInjectedStarknetAddress((value as { selectedAddress?: unknown }).selectedAddress) ||
      parseInjectedStarknetAddress((value as { account?: unknown }).account) ||
      parseInjectedStarknetAddress((value as { accounts?: unknown }).accounts) ||
      parseInjectedStarknetAddress((value as { result?: unknown }).result) ||
      parseInjectedStarknetAddress((value as { data?: unknown }).data)
    )
  }
  return null
}
