...
const RAW_API_BASE_URL = (process.env.NEXT_PUBLIC_BACKEND_URL || "http://127.0.0.1:8080").trim()
...
export interface ExecuteSwapResponse {
  ...
  ai_level_points_bonus_percent?: string | number
...
export interface PrepareAiPlanResponse {
  user: string
  agent_id: string
  operator: string
  plan_hash: string
...
export interface ApproveAiPlanResponse {
  plan_id: string
  tx_hash: string
}
...
export interface PrivacyCreateNoteResponse {
  note_secret: string
  note_commitment: string
  nullifier: string
}

export interface PrivacyNoirInputsResponse {
...
export interface NftMintStatusResponse {
...
const HIDE_MODE_PROVER_TIMEOUT_MS = 900000
...
function clearAuthScopedCaches() {
  notificationsInFlight.clear()
  notificationsCache.clear()
  portfolioBalanceInFlight = null
  portfolioBalanceCache = null
  portfolioAnalyticsInFlight = null
  portfolioAnalyticsCache = null
  portfolioOhlcvInFlight.clear()
  portfolioOhlcvCache.clear()
  rewardsPointsInFlight = null
  rewardsPointsCache = null
  leaderboardInFlight.clear()
  leaderboardCache.clear()
  stakePoolsInFlight = null
  stakePoolsCache = null
  limitOrdersInFlight.clear()
  limitOrdersCache.clear()
  ownedNftsInFlight = null
  ownedNftsCache = null
  onchainBalancesInFlight.clear()
  onchainBalancesCache.clear()
  leaderboardUserCategoriesInFlight.clear()
  leaderboardUserCategoriesCache.clear()
}

function bumpAuthScopeVersion() {
  authScopeVersion += 1
...
function setStoredAuthToken(token: string) {
  ...
  const previous = window.localStorage.getItem(AUTH_TOKEN_STORAGE_KEY)
  window.localStorage.setItem(AUTH_TOKEN_STORAGE_KEY, trimmed)
  if (previous !== trimmed) {
    bumpAuthScopeVersion()
  }
}
...
export async function executeSwap(payload: {
  ...
  const isHideV4Request =
    payload.hide_balance === true &&
    (payload.privacy?.note_version || "").trim().toLowerCase() === "v4"
  const sanitizedPayload = isHideV4Request ? { ...payload, recipient: undefined } : payload
  ...
  return apiFetch<ExecuteSwapResponse>("/api/v1/swap/execute", {
    ...
    suppressErrorNotification: true,
    timeoutMs: isHideV4Request ? HIDE_MODE_PROVER_TIMEOUT_MS : 120000,
  })
}
...
export async function createLimitOrder(payload: {
  ...
  return apiFetch<LimitOrderResponse>("/api/v1/limit-order/create", {
    ...
    suppressErrorNotification: true,
    timeoutMs: payload.hide_balance ? HIDE_MODE_PROVER_TIMEOUT_MS : 120000,
  })
}
...
export async function mintNft(payload: { tier: number; onchain_tx_hash?: string }) {
  return apiFetch<NFTItem | NftMintStatusResponse>("/api/v1/nft/mint", {
    method: "POST",
    body: JSON.stringify(payload),
    context: "Mint NFT",
    suppressErrorNotification: true,
...
export async function getNftMintStatus(txHash: string) {
  return apiFetch<NftMintStatusResponse>(`/api/v1/nft/status/${txHash}`, {
    method: "GET",
    context: "NFT mint status",
    suppressErrorNotification: true,
  })
}
...
export async function claimRewards() {
  return apiFetch<{ tx_hash?: string | null; amount_carel: number; points_converted: number; onchain: boolean }>(
...
/**
 * Runs `prepareAiPlan` as part of the frontend API client workflow.
 */
export async function prepareAiPlan(payload?: {
  action_mask?: number
  max_actions?: number
  expires_at?: number
  expiry_days?: number
  ...
  return apiFetch<PrepareAiPlanResponse>("/api/v1/ai/plan/prepare", {
    method: "POST",
    body: JSON.stringify(payload ?? {}),
    context: "AI prepare plan",
    suppressErrorNotification: true,
    timeoutMs: 45000,
  })
}
...
/**
 * Runs `approveAiPlan` as part of the frontend API client workflow.
 */
export async function approveAiPlan(payload: {
  user: string
  agent_id: string
  operator?: string
  plan_hash: string
  ...
  return apiFetch<ApproveAiPlanResponse>("/api/v1/ai/plan/approve", {
    method: "POST",
    body: JSON.stringify(payload),
    context: "AI approve plan",
    suppressErrorNotification: true,
    timeoutMs: 60000,
  })
}
...
/**
 * Runs `getAiPlanStatus` as part of the frontend API client workflow.
 */
export async function getAiPlanStatus(planId: string) {
  const params = new URLSearchParams({ plan_id: planId })
  return apiFetch<AiPlanStatusResponse>(`/api/v1/ai/plan/status?${params.toString()}`, {
    context: "AI plan status",
    suppressErrorNotification: true,
    timeoutMs: 20000,
  })
}
...
export async function autoSubmitPrivacyAction(payload?: {
  ...
  tx_context?: {
    ...
    approval_amount_low?: string
    approval_amount_high?: string
    payout_token?: string
    min_payout?: string
    min_payout_low?: string
    min_payout_high?: string
    note_commitment?: string
    note_deposit_tx_hash?: string
    denom_id?: string
    spendable_at_unix?: number
  ...
  return apiFetch<PrivacyAutoSubmitResponse>("/api/v1/privacy/auto-submit", {
    ...
    suppressErrorNotification: true,
    timeoutMs: HIDE_MODE_PROVER_TIMEOUT_MS,
  })
}
...
export async function preparePrivateExecution(payload: {
  ...
  action_calldata: string[]
  privacy_payload?: PrivacyVerificationPayload
  token?: string
  amount_low?: string
  amount_high?: string
  signature_selector?: string
...
export async function resolvePrivacyNoirInputs(
  payload?: {
  ...
  const endpoint = (options?.endpoint || "/api/v1/privacy/noir-inputs").trim() || "/api/v1/privacy/noir-inputs"
  return apiFetch<PrivacyNoirInputsResponse>(endpoint, {
    method: "POST",
    body: JSON.stringify(payload ?? {}),
    context: "Resolve noir inputs",
    suppressErrorNotification: true,
    timeoutMs: options?.timeoutMs ?? 30000,
  })
}

export async function createPrivacyNote(payload: {
  note_secret?: string
  note_amount: string
  note_token: string
}) {
  return apiFetch<PrivacyCreateNoteResponse>("/api/v1/privacy/note", {
    method: "POST",
    body: JSON.stringify(payload),
    context: "Create privacy note",
    suppressErrorNotification: true,
    timeoutMs: 30000,
  })
}
...
export interface PrivateBtcSwapInitiatePayload {
...
export interface PrivateBtcSwapInitiateResponse {
  swap_id: number
  deposit_address: string
  status: string
...
export interface PrivateBtcSwapFinalizePayload {
  swap_id: number
  btc_txid: string
...
export interface PrivateBtcSwapFinalizeResponse {
  tx_hash: string
  proof_cid: string
  proof_hash: string
  status: string
...
export interface PrivateBtcSwapOrderResponse {
  swap_id: number
  status: string
  deposit_address: string
  refund_address?: string | null
  btc_txid?: string | null
  amount_satoshi?: number | null
  note_commitment?: string | null
...
export async function initiatePrivateBtcSwap(payload: PrivateBtcSwapInitiatePayload) {
  return apiFetch<PrivateBtcSwapInitiateResponse>("/api/v1/private-btc-swap/initiate", {
    method: "POST",
    body: JSON.stringify(payload),
    context: "Initiate private BTC swap",
    timeoutMs: 30000,
  })
}

export async function finalizePrivateBtcSwap(payload: PrivateBtcSwapFinalizePayload) {
  return apiFetch<PrivateBtcSwapFinalizeResponse>("/api/v1/private-btc-swap/finalize", {
    method: "POST",
    body: JSON.stringify(payload),
    context: "Finalize private BTC swap",
    timeoutMs: 120000,
  })
}

export async function getPrivateBtcSwapOrder(swapId: number) {
  return apiFetch<PrivateBtcSwapOrderResponse>(`/api/v1/private-btc-swap/order/${swapId}`, {
    method: "GET",
    context: "Fetch private BTC swap order",
    timeoutMs: 30000,
  })
}
