...
import { usePriceStream } from "@/hooks/price/use-price-stream"
import {
...
async function fetchCoinGecko(ids: string[], apiKey?: string) {
  if (!ids.length) {
...
export function useLivePrices(tokens: string[], options: UseLivePricesOptions = {}): LivePriceState {
  ...
  const coingeckoTokenIds = React.useMemo(
    () =>
      uniqueTokens
        .map((token) => coingeckoIds[token])
        .filter((id): id is string => Boolean(id)),
    [uniqueTokens, coingeckoIds]
  )

  const coingeckoKey = enabled && useCoinGecko && coingeckoTokenIds.length > 0
    ? ["coingecko", coingeckoTokenIds.join(","), apiKey || ""]
    : null

  const { data: coingeckoData } = useSWR(
    coingeckoKey,
    () => fetchCoinGecko(coingeckoTokenIds, apiKey),
    {
      refreshInterval: enabled && useCoinGecko ? refreshMs : 0,
      revalidateOnFocus: true,
      revalidateOnReconnect: true,
      refreshWhenHidden: false,
      refreshWhenOffline: false,
      dedupingInterval: Math.min(refreshMs, 20000),
      keepPreviousData: true,
    }
  )

  React.useEffect(() => {
    if (!coingeckoData || !coingeckoData.data) return
    const now = Date.now()
    ...
    if (Object.keys(nextPrices).length > 0) {
      ...
      setLastRefresh(coingeckoData.ts)
    }
  }, [coingeckoData, coingeckoIds, staleMs, uniqueTokens])
