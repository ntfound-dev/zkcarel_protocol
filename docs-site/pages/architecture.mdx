...
```
L1:
  AI command → Backend response (no on-chain execution)

L2 (normal):
  AI command → AIExecutor setup on-chain
                     ↓
               Backend execute → Swap / Limit / Stake / Bridge

L3 (hide):
  AI command → AIExecutor setup on-chain
                     ↓
               User deposit note → Mixing window
                     ↓
               Backend execute → Relayer submit → ShieldedPool V4 → Swap / Limit / Stake
```
