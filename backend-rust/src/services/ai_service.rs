...
impl AIService {
        ...
        if command_lower.contains("swap") || command_lower.contains("exchange") {
            Ok(Intent {
                ...
                parameters: serde_json::json!({
                    "from": self.extract_token(&command_lower, "from"),
                    "to": self.extract_token(&command_lower, "to"),
                    "amount": self.extract_amount(&command_lower),
                }),
            })
            ...
            Ok(Intent {
                ...
                parameters: serde_json::json!({
                    "token": self.extract_token(&command_lower, "stake"),
                    "amount": self.extract_amount(&command_lower),
                }),
            })
        } else if command_lower.contains("market") || command_lower.contains("analysis") {
            // attempt to detect optional token for market analysis
            Ok(Intent {
                action: "market_analysis".to_string(),
                parameters: serde_json::json!({
                    "token": self.extract_token(&command_lower, "market"),
                }),
            })
        } else {
            Ok(Intent {
    ...
    fn extract_token(&self, text: &str, _context: &str) -> String {
    ...
    fn extract_amount(&self, text: &str) -> f64 {
        // Extract numeric amount (first numeric token)
