...
if [ -z "${PRICE_ORACLE_ADDRESS:-}" ] || [ "${PRICE_ORACLE_ADDRESS}" = "0x..." ]; then
  ...
  declare_out=$(sncast declare --network "$NET" --contract-name PriceOracle 2>&1) || {
  ...
  out=$(sncast deploy --network "$NET" --contract-name PriceOracle --constructor-calldata \
...
if [ -n "${SWAP_AGGREGATOR_ADDRESS:-}" ] && [ "${SWAP_AGGREGATOR_ADDRESS}" != "0x..." ]; then
  ...
  sncast invoke --network "$NET" --contract-address "$SWAP_AGGREGATOR_ADDRESS" --function set_price_oracle --calldata "$PRICE_ORACLE_ADDRESS"
fi

if [ -n "${ROUTER_ADDRESS:-}" ] && [ "${ROUTER_ADDRESS}" != "0x..." ]; then
  echo "Wiring PriceOracle to Router..."
  sncast invoke --network "$NET" --contract-address "$ROUTER_ADDRESS" --function set_price_oracle --calldata "$PRICE_ORACLE_ADDRESS"
fi
...
if [ -n "${TOKEN_CONFIGS}" ]; then
  ...
  for entry in "${TOKENS[@]}"; do
    ...
    if [ -n "${SWAP_AGGREGATOR_ADDRESS:-}" ] && [ "${SWAP_AGGREGATOR_ADDRESS}" != "0x..." ]; then
      sncast invoke --network "$NET" --contract-address "$SWAP_AGGREGATOR_ADDRESS" --function set_token_oracle_config --calldata "$token_addr" "$asset_id" "$decimals"
    fi

    if [ -n "${ROUTER_ADDRESS:-}" ] && [ "${ROUTER_ADDRESS}" != "0x..." ]; then
      sncast invoke --network "$NET" --contract-address "$ROUTER_ADDRESS" --function set_token_oracle_config --calldata "$token_addr" "$asset_id" "$decimals"
    fi
  done
fi
...
set_fallback_price() {
  ...
  sncast invoke --network "$NET" --contract-address "$PRICE_ORACLE_ADDRESS" \
    --function set_fallback_price --calldata "$token_addr" "$price_low" "$price_high"
