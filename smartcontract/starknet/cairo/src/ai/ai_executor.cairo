...
pub trait IAIExecutor<TContractState> {
    ...
    // Submits an action for a user using an approved plan (no user signature).
    ...
    // Upgrades caller to tier L2 by paying CAREL to the dev wallet.
    fn upgrade_to_l2(ref self: TContractState);
    // Upgrades caller to tier L3 by paying CAREL to the dev wallet.
    fn upgrade_to_l3(ref self: TContractState);
    // Returns current tier for a given user.
...
pub trait IAIExecutorAdmin<TContractState> {
    ...
    // Updates fee + burn config in one call (governance).
    ...
    // Updates chain id used in signature hashing after access-control checks.
    fn set_chain_id(ref self: TContractState, chain_id: felt252);
    // Updates burn address used for fee burning after access-control checks.
    fn set_burn_address(ref self: TContractState, burn_address: ContractAddress);
    // Updates plan router configuration after access-control checks.
...
pub mod AIExecutor {
    ...
    fn constructor(
        ...
        // Post-mainnet defaults (can be lowered on testnet via admin)
    ...
    // Executes a pending action without signature verification.
    // Used by batch execution after a single batch signature is verified.
    ...
    impl AIExecutorImpl of IAIExecutor<ContractState> {
        ...
        fn submit_action(
            ...
            if self.fee_enabled.read() {
                let tier = self.user_tiers.entry(caller).read();
                fee = tier_fee(@self, tier);
        ...
        fn submit_action_from_plan(
            ...
            if self.fee_enabled.read() {
                let tier = self.user_tiers.entry(user).read();
                let fee = tier_fee(@self, tier);
                assert!(fee > 0, "Tier not active");

                let token = IERC20Dispatcher { contract_address: self.carel_token.read() };
                let ok = token.transfer_from(user, get_contract_address(), fee);
                assert!(ok, "Fee transfer failed");
                let burn_amount = tier_burn_amount(@self, tier);
                assert!(burn_amount <= fee, "Burn exceeds fee");
                if burn_amount > 0 {
        ...
        fn upgrade_to_l2(ref self: ContractState) {
            ...
            let fee = self.level_2_price.read();
        ...
        fn upgrade_to_l3(ref self: ContractState) {
            ...
            let fee = self.level_3_price.read();
            assert!(fee > 0, "Tier price not set");
            let dev_wallet = self.fee_recipient.read();
            assert!(!dev_wallet.is_zero(), "Dev wallet required");
            let token = IERC20Dispatcher { contract_address: self.carel_token.read() };
