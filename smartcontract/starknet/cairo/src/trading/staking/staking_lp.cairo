...
pub mod LPStaking {
    ...
    impl LPStakingImpl of ILPStaking<ContractState> {
        ...
        fn claim_rewards(ref self: ContractState, pool_address: ContractAddress) {
            ...
            if fee_amount > 0 {
                let ok_fee = IERC20Dispatcher { contract_address: self.reward_token.read() }.transfer(
            ...
            let ok = IERC20Dispatcher { contract_address: self.reward_token.read() }.transfer(user, net_reward);
            assert!(ok, "Token transfer failed");
