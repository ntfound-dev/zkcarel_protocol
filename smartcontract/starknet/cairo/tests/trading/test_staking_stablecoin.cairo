...
use smartcontract::trading::staking::staking_stablecoin::{IStakingStablecoinDispatcher, IStakingStablecoinDispatcherTrait};
...
#[starknet::interface]
pub trait IERC20MockStablecoin<TContractState> {
...
#[starknet::contract]
mod ERC20MockStablecoin {
    use starknet::ContractAddress;
    #[storage]
    struct Storage {}
    #[abi(embed_v0)]
    impl IERC20MockStablecoinImpl of super::IERC20MockStablecoin<ContractState> {
        // Applies transfer after input validation and commits the resulting state.
        // Used in isolated test context to validate invariants and avoid regressions in contract behavior.
...
fn setup() -> (IStakingStablecoinDispatcher, ContractAddress, ContractAddress, ContractAddress) {
    ...
    let erc20_class = declare("ERC20MockStablecoin").unwrap().contract_class();
...
fn test_claim_rewards_fee_consume_does_not_revert() {
    let (dispatcher, usdt_addr, _, owner) = setup();
    let user: ContractAddress = 0x444.try_into().unwrap();
    let amount: u256 = 1000000;
    let start_time: u64 = 1000000;
    ...
    let before = dispatcher.calculate_rewards(user, usdt_addr);
    dispatcher.claim_rewards(usdt_addr);
    let after = dispatcher.calculate_rewards(user, usdt_addr);
