...
use smartcontract::trading::staking::staking_carel::{
...
use smartcontract::trading::staking::staking_carel::StakingCarel::{Staked, Unstaked, RewardsClaimed};
...
#[test]
// Test case: validates stake below minimum results in tier 0 (no rewards).
// Used in isolated test context to validate invariants and avoid regressions in contract behavior.
fn test_stake_below_minimum_sets_tier_zero() {
    let (staking, _, user, _) = setup_staking();
    ...
    let info = staking.get_stake_info(user);
    assert!(info.amount == low_amount, "Amount mismatch");
    assert!(info.tier == 0, "Tier should be 0");
}
...
fn test_claim_rewards_fee_consume_does_not_revert() {
    ...
    start_cheat_caller_address(staking.contract_address, admin);
    staking.set_reward_fee(10000, admin);
    stop_cheat_caller_address(staking.contract_address);
    ...
    start_cheat_block_timestamp(staking.contract_address, start_time + 60);
    staking.claim_rewards();

    let remaining = staking.calculate_rewards(user);
    assert!(remaining > 0, "Rewards should remain when fee absorbs all");
