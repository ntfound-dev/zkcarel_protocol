...
use smartcontract::trading::staking::staking_lp::{ILPStakingDispatcher, ILPStakingDispatcherTrait};
...
#[starknet::interface]
pub trait IERC20MockLP<TContractState> {
...
mod ERC20MockLP {
    ...
    struct Storage {}
    #[abi(embed_v0)]
    impl IERC20MockLPImpl of super::IERC20MockLP<ContractState> {
        // Applies transfer after input validation and commits the resulting state.
        // Used in isolated test context to validate invariants and avoid regressions in contract behavior.
...
fn setup() -> (ILPStakingDispatcher, ContractAddress, ContractAddress, ContractAddress) {
    ...
    let erc20_class = declare("ERC20MockLP").unwrap().contract_class();
...
fn test_claim_rewards_fee_consume_does_not_revert() {
    let (dispatcher, _, pool_id, owner) = setup();
    let user: ContractAddress = 0x222.try_into().unwrap();
    let amount: u256 = 10000;
    let start_time: u64 = 100000;
    ...
    let before = dispatcher.calculate_rewards(user, pool_id);
    dispatcher.claim_rewards(pool_id);
    let after = dispatcher.calculate_rewards(user, pool_id);
