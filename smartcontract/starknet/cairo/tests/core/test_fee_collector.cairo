...
fn setup_protocol() -> (IFeeCollectorDispatcher, ITreasuryDispatcher, ContractAddress, ContractAddress) {
    ...
    collector.set_authorized_router(admin, true);
    stop_cheat_caller_address(collector_addr);

    (collector, treasury, admin, token_addr)
}
...
fn test_collect_swap_fee_distribution() {
    let (collector, treasury, admin, token_addr) = setup_protocol();
    ...
    collector.collect_swap_fee(token_addr, swap_amount, lp_provider);
    ...
    assert_eq!(treasury.get_treasury_balance(token_addr), 0); 
}
...
fn test_update_rates_only_owner() {
    let (collector, _, admin, _) = setup_protocol();

    start_cheat_caller_address(collector.contract_address, admin);
    collector.update_fee_rates(50, 40, 20, 30, 20);
    stop_cheat_caller_address(collector.contract_address);
}
...
fn test_update_rates_attacker_fails() {
    let (collector, _, _, _) = setup_protocol();
...
fn test_bridge_fee_forwarding() {
    let (collector, _treasury, admin, token_addr) = setup_protocol();
    let provider: ContractAddress = 0x444.try_into().unwrap();
    let mut spy = spy_events();

    start_cheat_caller_address(collector.contract_address, admin);
    collector.collect_bridge_fee(token_addr, 10_000_u256, provider);
    stop_cheat_caller_address(collector.contract_address);
