...
// Used in isolated test context to validate invariants and avoid regressions in contract behavior.
fn setup() -> (ContractAddress, ContractAddress, ITreasuryDispatcher, ICarelTokenTestDispatcher, ContractAddress) {
    ...
    // 4. Set governance executor for treasury outflows
    start_cheat_caller_address(treasury_addr, admin);
    treasury.set_governance_executor(admin);
    stop_cheat_caller_address(treasury_addr);

    (admin, treasury_addr, treasury, token, token_addr)
}
...
fn test_full_treasury_flow() {
    let (admin, treasury_addr, treasury, token, token_addr) = setup();
    ...
    treasury.receive_fee(token_addr, fee_amount);
    
    // This utilizes the Treasury import for event construction
    let expected_event = Treasury::Event::FeeReceived(
        Treasury::FeeReceived { from: collector, token: token_addr, amount: fee_amount }
    );
    ...
    treasury.burn_excess(token_addr, burn_amount);
    ...
    let final_bal = treasury.get_treasury_balance(token_addr);
...
fn test_burn_quota_enforced() {
    ...
    start_cheat_caller_address(treasury_addr, admin);
    treasury.burn_excess(token_addr, excess_burn);
}
