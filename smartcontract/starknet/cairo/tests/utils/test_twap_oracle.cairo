...
use snforge_std::{
    ...
    start_cheat_block_timestamp, stop_cheat_block_timestamp, load, start_cheat_caller_address
};
...
fn test_constructor_initialization() {
    let (dispatcher, _owner) = deploy_oracle();
...
fn test_update_and_get_spot_price() {
    let (dispatcher, owner) = deploy_oracle();
...
fn test_price_deviation_calculation() {
    ...
    start_cheat_block_timestamp(dispatcher.contract_address, ts + 10);
