...
#[starknet::interface]
pub trait IERC20MockRewards<TContractState> {
...
#[starknet::contract]
mod ERC20MockRewards {
    use starknet::ContractAddress;
    #[storage]
    struct Storage {}
    #[abi(embed_v0)]
    impl IERC20MockRewardsImpl of super::IERC20MockRewards<ContractState> {
        // Applies transfer after input validation and commits the resulting state.
        // Used in isolated test context to validate invariants and avoid regressions in contract behavior.
...
fn deploy_escrow(admin: ContractAddress) -> IRewardsEscrowDispatcher {
    let erc20_class = declare("ERC20MockRewards").unwrap().contract_class();
