...
- `LimitOrderBook` is the contract name in `src/trading/dca_orders.cairo`.
...
flowchart TD
  ...
  C --> D[Agent identity (ERC-8004) + operator wallet]
  ...
  F --> L2[L2 normal]
  L2 --> L2A[AIExecutor submit_action (batch)]
  L2A --> L2B[Backend execute]
  L2B --> L2C[Bridge / Swap / Limit / Stake]

  F --> L3[L3 hide]
  L3 --> H1[Auto setup on-chain]
  H1 --> H2[User deposit note]
  H2 --> H3[Mixing window]
  H3 --> H4[Relayer submit]
  H4 --> H5[ShieldedPool V4]
...
- L2 runs normal bridge/swap/limit/stake via `AIExecutor` + backend execution.
- L3 hide runs private swap/limit/stake via note deposit + mixing window + relayer submit into `ShieldedPool V4` (target).
- AI usage fees (current on-chain): L2 fee `5` CAREL (burn `2`, fee_recipient `3`), L3 fee `10` CAREL (burn `3`, fee_recipient `7`).
...
`LimitOrderBook` stores user orders for normal (public) execution. Hide-mode limit orders run through `ShieldedPool V4` (target), not the legacy privacy router. Rewards behavior is not normal-only: normal and hide flows can both feed points/NFT logic at the runtime layer, while hide adds the note path before relayer execution. For AI, `L1` stays off-chain, `L2` covers normal bridge/swap/limit/stake execution through `AIExecutor`, and `L3` is reserved for private swap/limit/stake execution through the shielded pool. Bridge does not run on `L3` yet; users should stay on `L2` for bridge until the private bridge path exists. Direct hide flows can exit a note via `private_exit_v3` (requires a ZK proof), but the current AI hide path does not expose note exits in the AI UI.
...
- `src/rewards/point_token.cairo`: `ERC20Component`, `SRC5Component`
...
### Trading/Bridge
- `SwapAggregator` (`src/trading/swap/swap_aggregator.cairo`)
- `BridgeAggregator` (`src/legacy/bridge_aggregator.cairo`)
- `LimitOrderBook` (`src/trading/dca_orders.cairo`)
- `DarkPool` (`src/legacy/dark_pool.cairo`)

### Privacy (legacy router)
The V1/V2 privacy router is kept for backward compatibility. Current Hide Mode
execution uses `private_executor_lite` (ShieldedPoolV3+) plus `garaga_real_bls`.
- `ZkPrivacyRouter` (V1, `src/legacy/privacy/zk_privacy_router.cairo`)
- `PrivacyRouter` (V2, `src/legacy/privacy/privacy_router.cairo`)
- `PrivacyIntermediary` (`src/trading/privacy_intermediary.cairo`)
- `GaragaVerifierAdapter` (`src/legacy/privacy/garaga_verifier_adapter.cairo`)
- `VerifierRegistry` (`src/legacy/privacy/verifier_registry.cairo`)
- `ShieldedVault` (`src/legacy/privacy/shielded_vault.cairo`)
- `PrivatePayments` (`src/legacy/private_payments.cairo`)
- `AnonymousCredentials` (`src/legacy/anonymous_credentials.cairo`)
...
- `smartcontract` (starknet/cairo): `126/126` passed
