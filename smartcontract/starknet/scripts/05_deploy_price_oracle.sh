...
TOKEN_USDC_ADDRESS=${TOKEN_USDC_ADDRESS:-0x05a26f9680c5dc0c36dcf1670d7f51f24ba0080d15fedb7396d23a77bf5c1924}
TOKEN_USDT_ADDRESS=${TOKEN_USDT_ADDRESS:-0x07439bce89f5559b3f6aa1793291c5bb20c03adf5bac57debe4d7209c2cb053b}
...
if [ -z "${PRICE_ORACLE_ADDRESS:-}" ] || [ "${PRICE_ORACLE_ADDRESS}" = "0x..." ]; then
  ...
  declare_out=$(run_sncast sncast declare "${SNCAST_NET_ARGS[@]}" --contract-name PriceOracle 2>&1) || {
  ...
  out=$(run_sncast sncast deploy "${SNCAST_NET_ARGS[@]}" --contract-name PriceOracle --constructor-calldata \
...
if [ -n "${SWAP_AGGREGATOR_ADDRESS:-}" ] && [ "${SWAP_AGGREGATOR_ADDRESS}" != "0x..." ]; then
  ...
  run_sncast sncast invoke "${SNCAST_NET_ARGS[@]}" --contract-address "$SWAP_AGGREGATOR_ADDRESS" --function set_price_oracle --calldata "$PRICE_ORACLE_ADDRESS" >/dev/null
fi

if [ -n "${ROUTER_ADDRESS:-}" ] && [ "${ROUTER_ADDRESS}" != "0x..." ]; then
  echo "Wiring PriceOracle to Router..."
  run_sncast sncast invoke "${SNCAST_NET_ARGS[@]}" --contract-address "$ROUTER_ADDRESS" --function set_price_oracle --calldata "$PRICE_ORACLE_ADDRESS" >/dev/null
fi
...
if [ -n "${TOKEN_CONFIGS}" ]; then
  ...
  for entry in "${TOKENS[@]}"; do
    ...
    if [ -n "${SWAP_AGGREGATOR_ADDRESS:-}" ] && [ "${SWAP_AGGREGATOR_ADDRESS}" != "0x..." ]; then
      run_sncast sncast invoke "${SNCAST_NET_ARGS[@]}" --contract-address "$SWAP_AGGREGATOR_ADDRESS" --function set_token_oracle_config --calldata "$token_addr" "$asset_id" "$decimals" >/dev/null
    fi

    if [ -n "${ROUTER_ADDRESS:-}" ] && [ "${ROUTER_ADDRESS}" != "0x..." ]; then
      run_sncast sncast invoke "${SNCAST_NET_ARGS[@]}" --contract-address "$ROUTER_ADDRESS" --function set_token_oracle_config --calldata "$token_addr" "$asset_id" "$decimals" >/dev/null
    fi
  done
fi
...
set_fallback_price() {
  ...
  run_sncast sncast invoke "${SNCAST_NET_ARGS[@]}" --contract-address "$PRICE_ORACLE_ADDRESS" \
