...
run_sncast sncast invoke "${SNCAST_NET_ARGS[@]}" --contract-address "$AI_EXECUTOR_ADDRESS" --function set_signature_verification --calldata "$AI_SIGNATURE_VERIFIER_ADDRESS" 1 >/dev/null

# Grant CAREL burn permission for AI execution fees
echo "Granting AI executor CAREL burner role..."
run_sncast sncast invoke "${SNCAST_NET_ARGS[@]}" --contract-address "$CAREL_TOKEN_ADDRESS" --function set_burner --calldata "$AI_EXECUTOR_ADDRESS" >/dev/null

# Configure bridge adapters
echo "Configuring bridge adapters..."
if needs_deploy "${BRIDGE_AGGREGATOR_ADDRESS:-}"; then
  echo "Skipping bridge adapters (BRIDGE_AGGREGATOR_ADDRESS missing)"
else
  run_sncast sncast invoke "${SNCAST_NET_ARGS[@]}" --contract-address "$BRIDGE_AGGREGATOR_ADDRESS" --function set_provider_adapter --calldata 0x41544d51 "$ATOMIQ_ADAPTER_ADDRESS" >/dev/null
  run_sncast sncast invoke "${SNCAST_NET_ARGS[@]}" --contract-address "$BRIDGE_AGGREGATOR_ADDRESS" --function set_provider_adapter --calldata 0x47415244 "$GARDEN_ADAPTER_ADDRESS" >/dev/null
  run_sncast sncast invoke "${SNCAST_NET_ARGS[@]}" --contract-address "$BRIDGE_AGGREGATOR_ADDRESS" --function set_provider_adapter --calldata 0x4c535750 "$LAYERSWAP_ADAPTER_ADDRESS" >/dev/null
fi

# Configure Garaga adapter mode
echo "Configuring Garaga verification mode..."
if needs_deploy "${GARAGA_ADAPTER_ADDRESS:-}"; then
  echo "Skipping Garaga verification mode (GARAGA_ADAPTER_ADDRESS missing)"
else
  run_sncast sncast invoke "${SNCAST_NET_ARGS[@]}" --contract-address "$GARAGA_ADAPTER_ADDRESS" --function set_verification_mode --calldata "$GARAGA_VERIFICATION_MODE" >/dev/null
fi
...
case "$PRIVACY_VERIFIER_KIND" in
 ...
 if needs_deploy "${ZK_PRIVACY_ROUTER_ADDRESS:-}" || needs_deploy "${PRIVACY_ADAPTER_ADDRESS:-}"; then
   echo "Skipping privacy router verifier wiring (ZK_PRIVACY_ROUTER_ADDRESS or privacy adapter missing)"
 else
   run_sncast sncast invoke "${SNCAST_NET_ARGS[@]}" --contract-address "$ZK_PRIVACY_ROUTER_ADDRESS" --function set_verifier --calldata "$PRIVACY_ADAPTER_ADDRESS" >/dev/null
 fi
