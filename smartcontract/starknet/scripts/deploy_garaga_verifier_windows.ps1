...
if ($env:SHIELDED_POOL_V4_ADDRESS) {
    $swapVerifier = if ($env:SHIELDED_POOL_V4_SWAP_VERIFIER) { $env:SHIELDED_POOL_V4_SWAP_VERIFIER } else { $env:GARAGA_ADAPTER_ADDRESS }
    $limitVerifier = if ($env:SHIELDED_POOL_V4_LIMIT_VERIFIER) { $env:SHIELDED_POOL_V4_LIMIT_VERIFIER } else { $env:GARAGA_ADAPTER_ADDRESS }
    $stakeVerifier = if ($env:SHIELDED_POOL_V4_STAKE_VERIFIER) { $env:SHIELDED_POOL_V4_STAKE_VERIFIER } else { $env:GARAGA_ADAPTER_ADDRESS }
    if ($swapVerifier -and $limitVerifier -and $stakeVerifier) {
        Write-Host "Wiring ShieldedPool v4 verifiers..."
        Invoke-Sncast -WorkingDir $smartcontractDir -CmdArgs @(
            "--wait",
            "--wait-timeout", $WaitTimeout.ToString(),
            "--wait-retry-interval", $WaitRetryInterval.ToString(),
            ...
            "invoke",
            "--url", $env:RPC_URL,
            "--contract-address", $env:SHIELDED_POOL_V4_ADDRESS,
            "--function", "set_verifiers",
            "--calldata", $swapVerifier, $limitVerifier, $stakeVerifier
        ) | Out-Null
    }
}
